...
from models import (
    ...
    init_model,
    models,
    fit_estimator_with_cv,
    get_predictions,
)
from plot import (
    create_dataset_figure,
    create_decision_figure,
...
with st.sidebar:
    ...
    data_noise = st.slider(
        "Select noise ratio", min_value=0.05, max_value=1.0, step=0.05, value=0.3
    )
    data_ratio = st.slider(
        "Select train/test ratio", min_value=0.1, max_value=0.9, step=0.05, value=0.8
    )
    data_balance = st.slider(
        "Select balance ratio", min_value=0.0, max_value=1.0, step=0.05, value=0.5
    )
